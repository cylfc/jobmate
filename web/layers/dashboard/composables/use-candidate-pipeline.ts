...
const normalizeStages = (rawStages: unknown): CandidatePipelineStage[] => {
  const items = Array.isArray(rawStages)
    ? (rawStages as CandidatePipelineApiItem[])
    : [];
  ...
  const normalized: CandidatePipelineStage[] = DEFAULT_STAGE_ORDER.map(
    (id) => ({
      id,
      count: map.get(id) ?? 0,
    }),
...
const _useCandidatePipeline = () => {
  ...
  const fetchCandidatePipeline = async () => {
    ...
    try {
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Failed to fetch candidate pipeline";
...
export const useCandidatePipeline = createSharedComposable(
  _useCandidatePipeline,
);
