...
const normalizeAlert = (
  raw: DashboardAlertsApiItem,
  idx: number,
): DashboardAlert => {
  const id =
    typeof raw.id === "string" && raw.id.trim() ? raw.id : `alert-${idx}`;
  const type =
    typeof raw.type === "string" && raw.type.trim() ? raw.type : "unknown";
  const message =
    typeof raw.message === "string" && raw.message.trim()
      ? raw.message
      : "Action required";
  const actionUrl =
    typeof raw.actionUrl === "string" && raw.actionUrl.trim()
      ? raw.actionUrl
      : "/";
...
const _useDashboardAlerts = () => {
  ...
  const normalizeAlerts = (
    rawAlerts: DashboardAlertsApiItem[],
  ): DashboardAlert[] => {
    return rawAlerts
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Failed to fetch dashboard alerts";
