...
import {
  useDashboardApi,
  type UseRecentActivitiesOptions,
} from "@dashboard/utils/dashboard-api";
...
const safeType = (value: unknown): RecentActivityType =>
  typeof value === "string" && value.trim()
    ? (value as RecentActivityType)
    : "unknown";

const normalizeEvent = (
  raw: Partial<RecentActivityEvent> & Record<string, unknown>,
  idx: number,
): RecentActivityEvent => {
  const id =
    typeof raw.id === "string" && raw.id.trim() ? raw.id : `activity-${idx}`;
  const type = safeType(raw.type);
  const occurredAt = isIso(raw.occurredAt)
...
const _useRecentActivities = () => {
  ...
  const fetchRecentActivities = async (
    ...
    try {
      ...
      const normalized = raw.map((e, idx) =>
        normalizeEvent(
          e as Partial<RecentActivityEvent> & Record<string, unknown>,
          idx,
        ),
      );
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Failed to fetch recent activities";
  ...
  const loadMore = async (limit: number = 20) => {
    ...
    try {
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Failed to load more activities";
      error.value = errorMessage;
      throw e;
    } finally {
      loading.value = false;
