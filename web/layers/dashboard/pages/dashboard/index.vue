  ...
  <div class="space-y-6">
    ...
    <div v-if="canSeePipeline" class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <UCard>
        <template #header>
          <div class="space-y-1">
            <h3 class="text-base font-semibold text-default">
              ...
              {{ t("dashboard.pipeline.chart.bar-title") }}
            </h3>
        ...
        <ChartsBarChart :items="pipelineBarChartItems" height="280px" />
      </UCard>
      ...
      <UCard>
        <template #header>
          <div class="space-y-1">
            <h3 class="text-base font-semibold text-default">
              {{ t("dashboard.pipeline.title") }} -
              {{ t("dashboard.pipeline.chart.pie-title") }}
            </h3>
            <p class="text-sm text-muted">
...
const { normalized, loading: kpisLoading, fetchKpis } = useDashboardKpis();

const { jobs: activeJobsData, fetchActiveJobs } = useActiveJobs();

const { alerts: alertsData, fetchAlerts } = useDashboardAlerts();
...
const { stages: candidatePipelineStages, fetchCandidatePipeline } =
  useCandidatePipeline();

const { events: recentActivityEvents, fetchRecentActivities } =
  useRecentActivities();
...
const canSeeMatchingHealth = computed(
  () => role.value === DASHBOARD_ROLE.RECRUITER
);
