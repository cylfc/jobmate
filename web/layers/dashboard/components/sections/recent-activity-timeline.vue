        ...
        <div v-else class="space-y-5">
          <div v-for="group in groups" :key="group.key" class="space-y-3">
            ...
            <div class="space-y-3">
              ...
              >
                <div
                  class="mt-0.5 flex size-8 items-center justify-center rounded-full bg-muted"
                >
                  <UIcon
                    :name="item.icon || 'i-lucide-activity'"
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <p class="text-xs text-dimmed whitespace-nowrap">
                      {{ item.at }}
                    </p>
                  </div>
...
const titleForEvent = (e: RecentActivityEvent) => {
  ...
  const jobTitle =
    typeof meta.jobTitle === "string" ? meta.jobTitle : undefined;
  const candidates = Number.isFinite(Number(meta.candidates))
    ? Number(meta.candidates)
    : undefined;

  switch (e.type) {
    case "cv_uploaded":
      return candidateName
        ? t("dashboard.activity.types.cv-uploaded-with-name", {
            name: candidateName,
          })
        : t("dashboard.activity.types.cv-uploaded");
    case "job_saved":
      return jobTitle
        ? t("dashboard.activity.types.job-saved-with-title", {
            title: jobTitle,
          })
        : t("dashboard.activity.types.job-saved");
    case "matching_completed":
      return jobTitle
        ? t("dashboard.activity.types.matching-completed-with-title", {
            title: jobTitle,
          })
        : t("dashboard.activity.types.matching-completed");
    case "interview_scheduled":
      return candidateName && jobTitle
        ? t("dashboard.activity.types.interview-scheduled-with-name-title", {
            name: candidateName,
            title: jobTitle,
          })
...
const descriptionForEvent = (e: RecentActivityEvent) => {
  ...
  return t("dashboard.activity.types.matching-completed-desc", {
    count: candidates,
  });
};
...
const groups = computed(() => {
  ...
  if (by.today.length)
    out.push({
      key: "today",
      label: t("dashboard.activity.group.today"),
      items: by.today,
    });
  if (by.yesterday.length)
    out.push({
      key: "yesterday",
      label: t("dashboard.activity.group.yesterday"),
      items: by.yesterday,
    });
  if (by.other.length)
    out.push({
      key: "other",
      label: t("dashboard.activity.group.earlier"),
      items: by.other,
    });
  return out;
});
