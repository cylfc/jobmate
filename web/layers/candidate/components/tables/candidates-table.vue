      ...
      >
        ...
        <template #skills-cell="{ row }">
          <div class="flex flex-wrap gap-1">
            <UBadge
              v-for="(skill, index) in getSkillsFromDetailed(
                row.original,
              ).slice(0, 3)"
              :key="index"
              color="neutral"
        ...
        <template #experience-cell="{ row }">
          <span class="text-sm"
            >{{ row.original.experience }} {{ t("candidate.years") }}</span
          >
        </template>
...
interface Emits {
  (
    e: "view-detail" | "invite" | "delete" | "match-jobs" | "edit",
    candidate: Candidate,
  ): void;
  (
    e: "bulk-invite" | "bulk-delete" | "bulk-match-jobs",
    candidateIds: string[],
  ): void;
}
...
const getSkillsFromDetailed = (candidate: Candidate): string[] => {
  ...
  if (candidate.skillsDetailed && Array.isArray(candidate.skillsDetailed)) {
    return candidate.skillsDetailed
      .map((skill) => {
