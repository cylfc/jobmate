<template>
  <div class="space-y-4">
    ...
    <div v-for="(skill, index) in skills" :key="skill.id || index" class="border rounded-lg p-4 space-y-4">
      <div class="flex items-center justify-between">
        <span class="text-sm font-medium">
          {{ skill.name || t('candidate.create.skills.entry', { defaultValue: 'Skill' }) }} #{{ index + 1 }}
        </span>
        <UButton
          color="error"
          variant="ghost"
