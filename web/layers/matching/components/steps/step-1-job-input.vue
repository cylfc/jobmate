  ...
  <div class="space-y-6">
    ...
    <div class="space-y-4">
      <div v-if="selectedMode === JOB_INPUT_MODE.INPUT" class="space-y-4">
        <UFormField
          :label="t('matching.step-1.job-description')"
          name="description"
          class="w-full"
        >
          <UTextarea
      ...
      <div v-else-if="selectedMode === JOB_INPUT_MODE.UPLOAD" class="space-y-4">
        <UFormField
          :label="t('matching.step-1.upload-file')"
          name="file"
          class="w-full"
        >
          <UInput
      ...
      <div v-else-if="selectedMode === JOB_INPUT_MODE.LINK" class="space-y-4">
        <UFormField
          :label="t('matching.step-1.job-link')"
          name="link"
          class="w-full"
        >
          <UInput
      ...
      <div
        v-else-if="selectedMode === JOB_INPUT_MODE.DATABASE"
        class="space-y-4"
      >
        <UFormField
          :label="t('matching.step-1.select-from-database')"
          name="job"
          class="w-full"
        >
          <USelectMenu
    ...
    <div class="flex items-center justify-between pt-4">
      ...
      >
        {{
          isProcessing
            ? t("matching.step-1.processing")
            : t("matching.step-1.next-step")
        }}
      </UButton>
    </div>
  </div>
...
const tabs = computed(() => [
  {
    label: t("matching.step-1.input"),
    value: JOB_INPUT_MODE.INPUT,
    icon: "i-lucide-pencil",
  },
  {
    label: t("matching.step-1.upload"),
    value: JOB_INPUT_MODE.UPLOAD,
    icon: "i-lucide-file-up",
  },
  {
    label: t("matching.step-1.link"),
    value: JOB_INPUT_MODE.LINK,
    icon: "i-lucide-link",
  },
  {
    label: t("matching.step-1.database"),
    value: JOB_INPUT_MODE.DATABASE,
...
const canProceed = computed(() => {
  if (selectedMode.value === JOB_INPUT_MODE.INPUT)
    return jobDescription.value.trim().length > 0;
  if (selectedMode.value === JOB_INPUT_MODE.UPLOAD)
    return uploadedFile.value !== null;
  if (selectedMode.value === JOB_INPUT_MODE.LINK)
    return jobLink.value.trim().length > 0;
  if (selectedMode.value === JOB_INPUT_MODE.DATABASE)
...
const handleNext = async () => {
  ...
  if (selectedMode.value === JOB_INPUT_MODE.DATABASE && selectedJobId.value) {
    const selectedJob = jobsFromDatabase.value.find(
      (j: Job) => j.id === selectedJobId.value,
    );
    if (selectedJob) {
  ...
  if (
    (selectedMode.value === JOB_INPUT_MODE.INPUT &&
