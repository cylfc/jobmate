  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <UButton color="primary" icon="i-lucide-plus" @click="$emit('reset')">
...
interface Emits {
  ...
  (
    e: "update:matchings",
    matchings: (Matching & { candidateName?: string })[],
  ): void;
}
...
watch(
  ...
  (newMatchings) => {
    if (newMatchings && newMatchings.length > 0) {
      localMatchings.value = newMatchings as (Matching & {
        candidateName?: string;
      })[];
      console.log(
        "Step 4 - Updated matchings from props:",
        localMatchings.value,
      );
    }
  },
...
const loadMatchings = async () => {
  ...
  try {
    ...
    if (apiMatchings && apiMatchings.length > 0) {
      localMatchings.value = apiMatchings as (Matching & {
        candidateName?: string;
      })[];
      ...
      localMatchings.value = props.matchings as (Matching & {
        candidateName?: string;
      })[];
