<template>
  <div class="space-y-6">
    ...
    <!-- Create Modal -->
    <ModalsCreateJobModal v-model="showCreateModal" @submit="handleCreateJob" />
  </div>
</template>
...
const { jobs, loading, error, fetchJobs, addJob, removeJob, removeJobs } =
  useJobList();
...
const handleBulkDelete = async (jobIds: string[]) => {
  ...
  try {
    ...
    toast.add({
      ...
      description: t("job.success.bulk-delete-success-description", {
        count: jobIds.length,
      }),
      color: "success",
    });
