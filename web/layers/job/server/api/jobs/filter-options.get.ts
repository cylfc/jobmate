...
export default defineEventHandler(
  async (event): Promise<ApiResponse<JobFilterOptions>> => {
    try {
      ...
      if (authHeader) {
        try {
          ...
          logWarn(
            "Failed to fetch companies and locations from backend, using empty arrays",
            error,
            "jobs.filter-options",
          );
      ...
      logError(
        "Error in /api/jobs/filter-options.get.ts",
        error,
        "jobs.filter-options",
      );
