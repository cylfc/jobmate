    ...
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <UColorModeButton color="primary" variant="outline" size="sm" />
        </div>
      </div>
      ...
      <div class="space-y-4">
        <UFormField
          :label="t('setting.system-config.timezone.title')"
          name="timezone"
          class="w-full"
        >
          <USelectMenu
      ...
      <div class="space-y-4">
        <UFormField
          :label="t('setting.system-config.date-format.title')"
          name="dateFormat"
          class="w-full"
        >
          <USelectMenu
      ...
      <div class="space-y-4">
        <UFormField
          :label="t('setting.system-config.time-format.title')"
          name="timeFormat"
          class="w-full"
        >
          <USelectMenu
...
import {
  ...
  THEMES,
  LANGUAGES,
} from "@setting/constants/system-config.constants";
...
onMounted(async () => {
  await Promise.all([fetchConfig(), loadOptions()]);
  if (config.value) {
