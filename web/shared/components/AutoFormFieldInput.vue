<script setup lang="ts">
  ...
  import { zodToHtmlInputType } from '@shared/utils/auto-form'
  import { useAutoFormField } from '@shared/composables/useAutoFormField'
  
  const props = defineProps<FieldProps>()
  
  // Use useAutoFormField composable to avoid hydration issues
  const { fieldValue } = useAutoFormField(props.fieldName)
  
  const inputType = computed(() => {
    return props.config?.inputProps?.type || zodToHtmlInputType(props.zodItem) || 'text'
  })
  ...
  // Filter out showLabel from inputProps before passing to input
