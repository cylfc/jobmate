...
export class CandidateTransformer extends BaseTransformer<
  BackendCandidate,
  Candidate
> {
  /**
  ...
  transform(backend: BackendCandidate): Candidate {
    ...
    if (expectedSalary && typeof expectedSalary === "object") {
      // Ensure it has the correct structure
      if (
        !("min" in expectedSalary) ||
        !("max" in expectedSalary) ||
        !("currency" in expectedSalary)
      ) {
    ...
    if (!currentSalary || !expectedSalary) {
      if (Array.isArray(backend.experience)) {
        for (const exp of backend.experience) {
          if (
            exp.currentSalary &&
            typeof exp.currentSalary === "object" &&
            !currentSalary
          ) {
          ...
          if (
            exp.expectedSalary &&
            typeof exp.expectedSalary === "object" &&
            !expectedSalary
          ) {
    ...
    return {
      ...
      experience:
        Array.isArray(backend.experience) && backend.experience.length > 0
          ? (
              backend.experience.find((e) => e.years !== undefined) as {
                years?: number;
              }
            )?.years || 0
          : 0,
      ...
      educations: Array.isArray(backend.educations)
        ? (backend.educations as Candidate["educations"])
        : undefined,
      skillsDetailed: Array.isArray(backend.skillsDetailed)
        ? (backend.skillsDetailed as Candidate["skillsDetailed"])
        : undefined,
      workExperiences: Array.isArray(backend.workExperiences)
        ? (backend.workExperiences as Candidate["workExperiences"])
        : undefined,
      projects: Array.isArray(backend.projects)
        ? (backend.projects as Candidate["projects"])
        : undefined,
