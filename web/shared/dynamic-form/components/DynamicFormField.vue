<script setup lang="ts">
  ...
  const fieldComponent = computed(() => {
    ...
    if (normalizedConfig.value?.component) {
      const component = fieldRegistry.resolve(props.fieldInfo.zodItem, normalizedConfig.value)
      if (component) {
        console.log(`[DynamicFormField] Using config component for "${fieldName}"`, {
          component: typeof normalizedConfig.value.component === 'string' ? normalizedConfig.value.component : 'custom'
        })
        return component
      }
    }
    ...
    if (fieldType !== 'unknown') {
      ...
      if (componentName) {
        ...
        if (entry) {
          console.log(`[DynamicFormField] Found component by type for "${fieldName}"`, {
            type: fieldType,
            componentName
          })
          component = entry.component
        } else {
          console.warn(`DynamicFormField: Component "${componentName}" not found in registry for type "${fieldType}"`, {
            availableComponents: Array.from(fieldRegistry.getAll().keys())
          })
        }
      } else {
        console.warn(`DynamicFormField: No component mapping for type "${fieldType}"`, {
          fieldName,
          availableTypes: Object.keys(typeMap)
        })
      }
    } else {
      console.warn(`DynamicFormField: Field type is "unknown" for "${fieldName}"`, {
        zodItemTypeName: props.fieldInfo.zodItem._def?.typeName,
    ...
    if (!component) {
      console.log(`[DynamicFormField] Trying full resolve from zodItem for "${fieldName}"`)
      component = fieldRegistry.resolve(props.fieldInfo.zodItem, normalizedConfig.value)
      if (component) {
        console.log(`[DynamicFormField] Found component via full resolve for "${fieldName}"`)
      }
    }
    
    if (!component) {
      console.warn(`DynamicFormField: No component found for field "${fieldName}" with type "${fieldType}"`, {
      ...
      if (inputEntry) {
        console.warn(`DynamicFormField: Using input as fallback for "${fieldName}"`)
  ...
  const fieldError = computed(() => {
    ...
    const error = props.formErrors.find((e: any) => e.path === props.fieldInfo.name)
