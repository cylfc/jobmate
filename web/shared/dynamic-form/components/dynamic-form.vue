<script setup lang="ts" generic="T extends z.ZodObject<z.ZodRawShape>">
...
import {
  parseSchema,
  unwrapSchema,
} from "@shared/dynamic-form/utils/schema-parser";
...
const formStateRef =
  props.formState || ref<Record<string, any>>({ ...initialValues });
...
const { state: _state } = useDynamicForm(formStateRef);

// Use provided registry or default
const _registry = computed(() => props.fieldRegistry || fieldRegistry);

// Track form errors to force re-render of fields when errors change
const formErrorsRef = ref<Array<{ path: string; message: string }>>([]);
...
const onSubmit = async (event: FormSubmitEvent<Record<string, unknown>>) => {
  ...
  if (!result.success) {
    ...
    if (formRef.value && result.error?.issues) {
      ...
      const errorsArray = result.error.issues
        ...
        .filter((err: { path: string; message: string }) => err.path); // Only include errors with valid paths

      console.log("[DynamicForm] Setting errors in onSubmit", errorsArray);
      console.log(
        "[DynamicForm] Field names:",
        fields.value.map((f) => f.name),
      );
      console.log(
        "[DynamicForm] Error paths:",
        errorsArray.map((e) => e.path),
      );
      console.log(
        "[DynamicForm] Paths match:",
        errorsArray.every((err) =>
      ...
      if (typeof formRef.value.setErrors === "function") {
        ...
        console.log(
          "[DynamicForm] formRef.value immediately after setErrors:",
          {
            errors: formRef.value.errors,
            hasErrors: !!formRef.value.errors,
            errorsLength: formRef.value.errors?.length || 0,
        ...
        console.log("[DynamicForm] Current form errors after nextTick", {
          ...
          errorsMatch: errorsArray.every((err) =>
            currentErrors.some(
              (ce: { path: string; message: string }) => ce.path === err.path,
            ),
          ),
        ...
        console.log(
          "[DynamicForm] Updated formErrorsRef:",
          formErrorsRef.value,
        );
      } else {
...
const onError = async (event: {
  ...
  if (!result.success && formRef.value && result.error?.issues) {
    ...
    if (typeof formRef.value.setErrors === "function") {
      ...
      const currentErrors = formRef.value.errors || [];
      console.log(
        "[DynamicForm] Current form errors after setErrors in onError",
        {
          errorCount: currentErrors.length,
          errors: currentErrors,
          errorsMatch: errorsArray.every((err) =>
