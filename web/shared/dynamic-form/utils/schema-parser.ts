 ...
 */
export function unwrapSchema<T extends z.ZodObject<any, any>>(
  schema: z.ZodAny
): z.ZodObject<any, any> {
  if (!schema) {
  ...
  console.warn('DynamicForm: Could not unwrap schema, returning as is', { typeName, schema })
  return schema as z.ZodObject<any, any>
}
 ...
 */
export function parseSchema(schema: z.ZodObject<any>): FieldInfo[] {
  ...
  for (const key in shape) {
    ...
    const description = (zodItem._def as any)?.description
    ...
    if (baseType === 'object' && zodItem._def?.typeName === 'ZodObject') {
      nestedFields = parseSchema(zodItem as z.ZodObject<any>)
    }
